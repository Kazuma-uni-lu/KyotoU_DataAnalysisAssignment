...
#print("Number of PCA components:", X_train_pca_2.shape[1])
...
#Since I already did the preprocessing in FP-6 I will just compare it to a result without it
def no_preprocessing_compared():
    X_raw = X.values
    X_train, X_test, y_train, y_test = train_test_split(
    ...
    y_pred_raw_1 = knn.predict(X_test)
    rmse_raw_1 = mean_squared_error(y_test, y_pred_raw_1, squared=False)
    r2_raw_1   = r2_score(y_test, y_pred_raw_1)
    print("KNN-regression results: ",rmse_raw_1, r2_raw_1)

    svr_raw = svm.SVR(kernel="rbf", C=100, gamma="scale", epsilon=0.1)
    svr_raw.fit(X_train, y_train)
    y_pred_raw = svr_raw.predict(X_test) 
    rmse_raw = mean_squared_error(y_test, y_pred_raw, squared=False)
    r2_raw   = r2_score(y_test, y_pred_raw)
    print("SVR-regression results: ",rmse_raw, r2_raw)

    labels_raw = KMeans(n_clusters=3, random_state=42).fit_predict(X_raw)
    sil_raw = silhouette_score(X_raw, labels_raw)
    print("Kmeans result: ",sil_raw)
